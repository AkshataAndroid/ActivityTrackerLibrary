...
public class DeviceInfo {
    ...
    DeviceInfo(Context context, ActivityInstanceConfig config, String trackerID) {
        ...
        initDeviceID(trackerID);
    }
    DeviceInfo(Context context,  String trackerID) {
        this.context = context;
        //  this.config = config;
        this.library = null;
        Thread deviceInfoCacheThread = new Thread(new Runnable() {
    ...
    private void initDeviceID(String trackerId) {

        //Show logging as per Manifest flag
        if(config.getEnableCustomTrackerId()){
            if(trackerId == null){
                config.getLogger().info("App_USE_CUSTOM_ID has been specified in the AndroidManifest.xml/Instance Configuration. App SDK will create a fallback device ID");
                recordDeviceError("App_USE_CUSTOM_ID has been specified in the AndroidManifest.xml/Instance Configuration. App SDK will create a fallback device ID");
            }
        }else{
            if(trackerId != null){
                config.getLogger().info("App_USE_CUSTOM_ID has not been specified in the AndroidManifest.xml. Custom App ID passed will not be used.");
                recordDeviceError("App_USE_CUSTOM_ID has not been specified in the AndroidManifest.xml. Custom App ID passed will not be used.");
            }
        }

        String deviceID = _getDeviceID();
        if(deviceID != null && deviceID.trim().length() > 2){
            getConfigLogger().verbose(config.getAccountId(),"App ID already present for profile");
            if(trackerId != null) {
                getConfigLogger().info(config.getAccountId(),"App ID - "+deviceID+" already exists. Unable to set custom App ID - " + trackerId);
                recordDeviceError("App ID - "+deviceID+" already exists. Unable to set custom App ID - " + trackerId);
            }
            return;
        }

        if(this.config.getEnableCustomTrackerId()) {
            forceUpdateCustomAppID(trackerId);
            return;
        }
...
//        }
        ...
        Thread generateGUIDFromAdIDThread = new Thread(new Runnable() {
            ...
            public void run() {
                fetchGoogleAdID();
                generateDeviceID();
                TrackerApi.instanceWithConfig(context,config).deviceIDCreated(getDeviceID());
            }
        });
        generateGUIDFromAdIDThread.start();
    }

    void forceUpdateCustomAppID(String AppID){
        if(Utils.validateCTID(AppID)){
            getConfigLogger().info(config.getAccountId(), "Setting App ID to custom App ID : " + AppID);
            forceUpdateDeviceId(Constants.CUSTOM_TRACKER_ID_PREFIX+AppID);
        }else {
            setOrGenerateFallbackDeviceID();
            removeDeviceID();
            getConfigLogger().info(config.getAccountId(),"Attempted to set invalid custom App ID - "+AppID+", falling back to default error App ID - "+getFallBackDeviceID());
            recordDeviceError("Attempted to set invalid custom App ID - "+AppID+", falling back to default error App ID - "+getFallBackDeviceID());
        }
    }
