...
public class DisplayUnitController {
    private final HashMap<String, DisplayUnit> items = new HashMap<>();
     ...
     * @return ArrayList<AppDisplayUnit> - could be null in case of null/empty/invalid json array
     */
    @Nullable
    public ArrayList<DisplayUnit> updateDisplayUnits(JSONArray messages) {

        //flush existing display units before updating with the new ones.
        reset();

        if (messages != null && messages.length() > 0) {
            final ArrayList<DisplayUnit> list = new ArrayList<>();
            try {
                for (int i = 0; i < messages.length(); i++) {
                    //parse each display Unit
                    DisplayUnit item = DisplayUnit.toDisplayUnit((JSONObject) messages.get(i));
     ...
     *
     * @return ArrayList<AppDisplayUnit> - Could be null in case no Display Units are there in the cache
     */
    @Nullable
    public ArrayList<DisplayUnit> getAllDisplayUnits() {
        if (!items.isEmpty()) {
     ...
     *
     * @param unitId - unitID of the Display Unit {@link DisplayUnit#getUnitID()}
     * @return AppDisplayUnit - Could be null in case no Display Units with the ID is found
     */
    @Nullable
    public DisplayUnit getDisplayUnitForID(String unitId) {
