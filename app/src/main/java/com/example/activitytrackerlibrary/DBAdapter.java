...
//        PUSH_NOTIFICATION_VIEWED("notificationViewed");
    ...
    private static final String DATABASE_NAME = "ActivityTrackerLibrary";
...
//                    " (" + KEY_CREATED_AT + ");";
    ...
    private static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onCreate(SQLiteDatabase db) {
            Logger.v("Creating ActivityTracker DB");
...
//            db.execSQL(NOTIFICATION_VIEWED_INDEX);
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            Logger.v("Upgrading ActivityTracker DB to version " + newVersion);
//            switch (oldVersion){
//                case 1 :
...
//            }
    ...
    private static String getDatabaseName(ActivityInstanceConfig config) {
        return config.isDefaultInstance() ? DATABASE_NAME : DATABASE_NAME + "_" + config.getAccountId();
    }

    DBAdapter(Context context, ActivityInstanceConfig config){
    ...
    synchronized int storeObject(JSONObject obj, Table table) {
        ...
        try {
            ...
            cv.put(KEY_CREATED_AT, sdf.format(resultdate));
...
//    }
     ...
     * @param inboxMessages ArrayList of type {@link TrackerMessageDAO}
     *
     */
//    synchronized void upsertMessages(ArrayList<TrackerMessageDAO> inboxMessages){
//        if (!this.belowMemThreshold()) {
//            Logger.v("There is not enough space left on the device to store data, data discarded");
//            return;
//        }
...
//        try {
//            final SQLiteDatabase db = dbHelper.getWritableDatabase();
//            for(TrackerMessageDAO messageDAO : inboxMessages) {
//                final ContentValues cv = new ContentValues();
...
//    }
     ...
     * @return ArrayList of {@link TrackerMessageDAO}
     */
//    synchronized ArrayList<TrackerMessageDAO> getMessages(String userId){
//        final String tName = Table.INBOX_MESSAGES.getName();
//        Cursor cursor;
//        ArrayList<TrackerMessageDAO> messageDAOArrayList = new ArrayList<>();
//        try{
//            final SQLiteDatabase db = dbHelper.getWritableDatabase();
//            cursor= db.rawQuery("SELECT * FROM "+tName+" WHERE " + USER_ID + " = ? ORDER BY " + KEY_CREATED_AT+ " DESC", new String[]{userId});
//            if(cursor != null) {
//                while(cursor.moveToNext()){
//                    TrackerMessageDAO tMessageDAO = new TrackerMessageDAO();
//                    tMessageDAO.setId(cursor.getString(cursor.getColumnIndex(_ID)));
//                    tMessageDAO.setJsonData(new JSONObject(cursor.getString(cursor.getColumnIndex(KEY_DATA))));
//                    tMessageDAO.setWzrkParams(new JSONObject(cursor.getString(cursor.getColumnIndex(WZRKPARAMS))));
//                    tMessageDAO.setDate(cursor.getLong(cursor.getColumnIndex(KEY_CREATED_AT)));
//                    tMessageDAO.setExpires(cursor.getLong(cursor.getColumnIndex(EXPIRES)));
//                    tMessageDAO.setRead(cursor.getInt(cursor.getColumnIndex(IS_READ)));
//                    tMessageDAO.setUserId(cursor.getString(cursor.getColumnIndex(USER_ID)));
//                    tMessageDAO.setTags(cursor.getString(cursor.getColumnIndex(TAGS)));
//                    tMessageDAO.setCampaignId(cursor.getString(cursor.getColumnIndex(CAMPAIGN)));
//                    messageDAOArrayList.add(tMessageDAO);
//                }
//                cursor.close();
//            }
