...
import static com.example.activitytrackerlibrary.StorageHelper.getPreferences;
...
public class InAppFCManager {
    ...
    InAppFCManager(Context context, ActivityInstanceConfig config, String deviceId){
        this.config = config;
        this.context = context;
        this.deviceId = deviceId;
        init(deviceId);
    }
    InAppFCManager(Context context,  String deviceId){
        this.config = config;
        this.context = context;
        this.deviceId = deviceId;
        init(deviceId);
    ...
    boolean canShow(InAppNotification inapp) {
        try {
    ...
    void didShow(final Context context, InAppNotification inapp) {
        final String id = getInAppID(inapp);
        if (id == null) return;
    ...
    void didDismiss(InAppNotification inapp) {
    ...
    private String getInAppID(InAppNotification inapp) {
    ...
    private boolean hasSessionCapacityMaxedOut(InAppNotification inapp) {
        final String id = getInAppID(inapp);
    ...
    private boolean hasLifetimeCapacityMaxedOut(InAppNotification inapp) {
        final String id = getInAppID(inapp);
        if (id == null) return false;
    ...
    private boolean hasDailyCapacityMaxedOut(InAppNotification inapp) {
        final String id = getInAppID(inapp);
        if (id == null) return false;
